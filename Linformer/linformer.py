...
class LinformerSelfAttention(nn.Module):
    def __init__(self, dim, seq_len, k=256, heads=8,
                 dim_head=None, one_kv_head=False,
                 share_kv=False, dropout=0.):
        super().__init__()
    ...
    def forward(self, x, context=None, **kwargs):
        ...
        def proj_seq_len(args): return torch.einsum(
            'b n d, n k -> b k d', *args)
        ...
        dots = torch.einsum('b h n d, b h k d -> b h n k',
                            queries, keys) * (d_h ** -0.5)
        attn = dots.softmax(dim=-1)
        attn = self.dropout(attn)
        out = torch.einsum('b h n k, b h k d -> b h n d', attn, values)
...
class LinformerLM(nn.Module):
    def __init__(self,
        ...
        self.linformer = Linformer(
            dim, seq_len, depth, k, heads, dim_head,
            one_kv_head, share_kv, reversible, dropout
        )
